...
backup_lock_file="$qfc_lock_root/backup.lock"
prepare_lock_file "$maintenance_lock_file" || {
  echo "The QFieldCloud maintenance lock file is missing or unsafe." >&2
  exit 1
}
prepare_lock_file "$backup_lock_file" || {
  echo "The QFieldCloud backup lock file is missing or unsafe." >&2
  exit 1
}
...
if ! lock_fd_matches_file 9 "$backup_lock_file"; then
  echo "The QFieldCloud backup lock descriptor changed unexpectedly." >&2
  exit 1
fi
if ! flock -n 9; then
...
write_recovery_required "backup-maintenance-in-progress" || {
  echo "The durable recovery-required marker could not be created; no service was stopped." >&2
  exit 1
}
...
remove_owned_recovery_marker || {
  echo "Services recovered, but the durable recovery-required marker could not be removed safely." >&2
  exit 1
}
services_quiesced="false"
