...
`lab-lightsail` 템플릿과 설치 스크립트는 구현되어 있지만 실제 AWS 검증 전입니다. 설치 명령은 [현재 파일럿 안내서](../lab-lightsail.md)를 사용합니다. 아래 항목은 두 배포 프로필이 계속 지켜야 할 공통 안전 순서이며, `standard-aws` 실행 도구는 아직 없습니다.
