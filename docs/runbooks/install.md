...
> 실제 새 AWS 계정에서 생성부터 삭제까지 이어지는 종단 간 시험은 아직 수행하지 않았습니다. 아래 작업은 월 최대 US$24인 Lightsail 서버를 만들 수 있습니다.
...
5. 기본값을 유지한 채 마지막 화면에서 **Submit**을 누릅니다.
6. 최대 150분 동안 기다립니다. `CREATE_COMPLETE`가 되면 **Outputs → HttpsUrl**을 엽니다.
...
사용할 파일은 [`releases/lab-lightsail/v0.1.0/template.yaml`](../../releases/lab-lightsail/v0.1.0/template.yaml)입니다. [`infra/lab-lightsail/template.yaml`](../../infra/lab-lightsail/template.yaml)은 릴리스 제작용 원본 틀이므로 직접 올리지 않습니다.
