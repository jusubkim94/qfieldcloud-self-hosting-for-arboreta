...
2026-08-24 실제 AWS `v0.1.2` 시험에서 공인 IP 인증서 발급·Nginx 적용·신뢰된 HTTPS 확인과 자동 갱신 timer 설치까지 성공했고, `v0.1.3` 자체서명 시험은 전체 설치 완료까지 통과했습니다. 시간 경과 후의 실제 인증서 자동 갱신과 `v0.1.4` 공인 인증서 전체 경로는 아직 검증 전이므로, 아래 공인 인증서 로그 절차는 해당 모드 서버에만 적용합니다.
