...
Certbot `5.7.0` 이미지와 자동화 코드는 정적 계약 검사 대상이며, 실제 AWS에서 Let’s Encrypt 최초 발급·Nginx 적용·신뢰된 HTTPS와 갱신 timer 설치까지 확인했습니다. 시간 경과 후 실제 자동 갱신과 `v0.1.4` 전체 설치 재시험은 아직 남아 있습니다. 라이선스 기록은 기술 검증 완료나 서비스 가용성 보증을 뜻하지 않습니다.
