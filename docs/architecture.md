...
현재 다운로드용 `v0.1.4` 완성 템플릿은 저장소에 포함되어 있습니다. `v0.1.3` 자체서명 경로는 실제 AWS `CREATE_COMPLETE`까지 통과했지만, 기본 Let’s Encrypt 공인 IP 인증서를 사용하는 `v0.1.4` 재시험과 성공 스택 삭제는 아직 남아 있습니다.
