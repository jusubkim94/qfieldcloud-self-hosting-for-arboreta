...
현재 다운로드용 `v0.1.4` 완성 템플릿은 저장소에 포함되어 있습니다. `v0.1.3` 자체서명 경로와 `v0.1.4` 기본 Let’s Encrypt 공인 IP 인증서 경로가 각각 실제 AWS `CREATE_COMPLETE`까지 통과했습니다. `v0.1.4` 시험은 CloudShell 생성 경로였으며 브라우저 파일 업로드 화면과 시간 경과 인증서 갱신은 검증 범위에 포함하지 않았습니다. 자세한 근거는 [v0.1.4 실제 AWS 검증 기록](verification/lab-lightsail-v0.1.4-2026-08-25.md)에 있습니다.
