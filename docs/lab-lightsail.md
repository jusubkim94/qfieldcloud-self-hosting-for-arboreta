...
- **완료:** 템플릿, PowerShell과 Bash 문법, 고정 버전, Docker Compose 구성 및 민감정보 노출 여부를 포함한 정적 검증. 서울 리전의 실제 AWS 파일럿에서 commit `afea815a35bb69c8a0d97cddb5eaab92c4686986`의 QFieldCloud `v26.25` 설치, QGIS `3.44.13-Solothurn` worker 연쇄 시험, 최초 백업, 최신 백업의 격리 DB·객체 저장소 복원시험과 전체 상태를 확인함
- **완료:** 실제 스택 `CREATE_COMPLETE`, 인스턴스 실행, 삭제 방지 활성화, 자체서명 인증서의 호스트 이름·SHA-256 지문·유효기간 일치, 외부 HTTPS 상태 주소의 DB·객체 저장소 `ok`를 확인함
- **완료:** 비루트 관리자 권한 준비와 분리된 설치자의 1시간 배포 역할 흐름에 대한 로컬 계약 검사, 실제 AWS CloudFormation 템플릿 문법 검증
- **미완료:** Lightsail 시작 스크립트의 표준입력을 하위 설치 명령과 분리하는 마지막 자동 완료 보고 수정의 빈 서버 재배포, 새 계정에서 권한 준비부터 역할 전환까지의 종단 간 시험, 실제 QField 앱 접속, 부하, 장애와 삭제 시험

실제 배포에서는 내부 설치와 모든 검증이 끝났지만, 하위 설치 명령이 표준입력에 남은 CloudFormation 완료 구문을 소비해 자동 완료 보고가 한 번 누락됐습니다. 전체 상태를 다시 검증한 뒤 같은 WaitCondition(완료 대기 조건)에 성공 신호를 수동 보완했습니다. 현재 실행 중인 파일럿은 정상이나, 이 원인을 막는 `</dev/null` 수정 자체는 아직 새 빈 서버에서 처음부터 검증하지 않았습니다. 실제 AWS에서 실행하지 않은 범위는 성공했다고 보지 않습니다.
