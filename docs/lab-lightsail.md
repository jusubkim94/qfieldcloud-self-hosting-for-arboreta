...
- **완료:** 서울 리전의 새 빈 서버에서 commit `6288b495f49531b207546805fd1b1758dac940da`의 QFieldCloud `v26.25`를 처음부터 배포하여, 수동 신호 보완 없이 실제 스택 `CREATE_COMPLETE`와 `BootstrapStatus=verified-by-wait-condition`을 확인함
- **완료:** 기본 `self-signed` 모드에서 인스턴스 실행, 삭제 방지 활성화, 자체서명 인증서의 호스트 이름·SHA-256 지문·유효기간 일치, 외부 HTTPS의 DB·객체 저장소 `ok`, 서버 내부 전체 상태 `overall=ok`, QGIS `3.44.13-Solothurn` worker의 프로젝트 처리·패키징·임시 컨테이너 정리 `passed`를 확인함
- **완료:** 최초 로컬 백업과 checksum 검사 `passed-local-only`, 최신 백업의 격리 복원시험 `schema-storage-integrity-passed`, `restore_matches_latest=true`, 검사 대상으로 표시된 임시 복원 Docker 자원 없음(`restore_test_orphans=clear`)을 확인함
- **완료:** 비루트 관리자 권한 준비와 분리된 설치자의 1시간 배포 역할 흐름에 대한 로컬 계약 검사, 실제 AWS CloudFormation 템플릿 문법 검증
- **구현·미검증:** 선택 모드 `letsencrypt-ip`의 Let’s Encrypt 공인 IPv4 인증서 발급·자동 갱신·롤백 코드와 AWS를 호출하지 않는 정적 계약 검사. **현재 AWS 스택에는 적용하지 않았고 실제 최초 발급·갱신 종단 간 시험도 아직 완료하지 않음**
- **미완료:** 새 계정에서 권한 준비부터 역할 전환까지의 종단 간 시험, 실제 QField 앱 접속, 부하와 장애 시험, 권장 역할 기반 관리자 삭제 흐름의 종단 간 시험, 서버 밖 백업을 이용한 별도 서버 재해 복구 시험

이전 파일럿에서는 내부 설치와 모든 검증이 끝난 뒤 하위 설치 명령이 표준입력에 남은 CloudFormation 완료 구문을 소비해 성공 신호를 수동 보완해야 했습니다. `</dev/null`로 하위 명령의 입력을 분리한 수정본은 2026-08-23 실제 빈 서버의 **`self-signed` 배포**에서 같은 WaitCondition(완료 대기 조건)에 자동 성공 신호를 보내고 `CREATE_COMPLETE`가 되는 것까지 확인했습니다. 위 로컬 백업과 격리 복원시험은 현재 서버 안에서 수행한 검사이므로 서버 자체를 잃는 재해에 대비한 별도 서버 복구 검증은 아닙니다. `letsencrypt-ip`을 포함해 실제 AWS에서 실행하지 않은 나머지 범위는 성공했다고 보지 않습니다.
...
| TLS 인증서 | 기본 `self-signed`: `IP.sslip.io`용 365일 자체서명 인증서. 선택 `letsencrypt-ip`: 고정 IPv4용 Let’s Encrypt 160시간 short-lived 인증서와 자동 갱신 |
| 자동 snapshot(디스크 시점 사본) | 기본 활성화, 매일 18:00 UTC(한국 다음 날 03:00)에 시작 |
| 경보 | 상태검사 실패와 CPU 80% 경보, 알림 수신자는 자동 설정하지 않음 |
| 설치 완료 대기 | 공인 인증서 발급 가능 시간을 포함해 bootstrap 본 작업은 최대 100분이며 제한 도달 시 안전한 종료·서비스 복구에 최대 10분을 추가로 허용함. CloudFormation 완료 신호는 150분까지 기다리고, 로컬 배포 스크립트는 그보다 긴 약 170분까지 전체 스택 상태를 감시함 |
| 삭제 방지 | 생성 시작부터 CloudFormation termination protection 활성화 |
| 업데이트 방지 | 생성 시 stack policy로 모든 CloudFormation 업데이트 거부 |

서버 안에는 QFieldCloud app·Nginx·worker wrapper, QFieldCloud 전용 PostGIS, RustFS 객체 저장소, 메모리 캐시, 시험용 메일 수신기와 작업 스케줄러가 Docker Compose로 함께 실행됩니다. `letsencrypt-ip`을 선택하면 Docker socket이나 별도 공개 포트가 없는 유지관리 전용 Certbot 서비스도 필요할 때만 실행됩니다. 기존 식물이력관리 PostGIS에는 연결하지 않습니다. 전체 흐름은 [스크립트 구조 그림](diagrams/lab-lightsail-script-structure.md)에 있습니다.
...
`letsencrypt-ip`은 기존 인스턴스의 HTTP 80과 HTTPS 443만 사용하므로 **추가 AWS 자원과 추가 AWS 직접 비용을 만들지 않습니다.** 다만 Let’s Encrypt 인증기관(CA), 공개 인터넷 연결과 발급 rate limit(일정 시간의 요청 제한)에 새로 의존합니다. 인증서 서비스가 무료라는 사실은 인스턴스·snapshot·전송 비용을 없애지 않습니다.
...
6. `letsencrypt-ip`을 선택한다면 Let’s Encrypt 이용약관 검토·동의, 고정 IPv4의 HTTP 80 공개와 외부 CA 의존을 받아들일 것
...
pwsh -NoProfile -File .\scripts\lab-lightsail\Test-CertificateAutomation.ps1
git remote get-url origin
git status --short
git log -1 --format=%H
```

앞의 네 시험 명령은 AWS API를 호출하지 않고 배포·삭제 정책, 권한 준비, 역할 프로필, 고정 스택 이름, 템플릿 자원 형식과 인증서 자동화의 안전 계약을 검사합니다. 이 정적 검사는 실제 Let’s Encrypt 발급·갱신 성공을 증명하지 않습니다. `git status --short`는 아무것도 출력하지 않아야 합니다. 수정사항이 있거나 아직 Push하지 않은 작업이라면 배포하지 말고 정상적인 Pull Request 검토와 병합 절차를 먼저 끝냅니다.
...
기본값은 검증 이력이 있는 `self-signed`입니다. 공인 IPv4 인증서 선택 기능까지 계획에 넣으려면 먼저 Let’s Encrypt 이용약관을 읽고 다음 두 옵션을 함께 추가합니다. `-AcceptLetsEncryptTerms`는 사용자가 약관을 검토하고 동의했다는 기록이며 자동 동의를 뜻하지 않습니다.
...
pwsh -NoProfile -File .\scripts\lab-lightsail\Install-QFieldCloudPilot.ps1 `
  ...
  -RoleProfile qfc-lab-role `
  -ExpectedAccountId '<12자리 계정 ID>' `
  -CertificateMode letsencrypt-ip `
  -AcceptLetsEncryptTerms
```
...
- `CertificateMode`가 선택한 `self-signed` 또는 `letsencrypt-ip`
- `letsencrypt-ip`이면 `CertificateEndpoint=static-ipv4`, `CertificateLifetimeHours=160`, `CertificateRenewalCheck=six-hours-plus-random-delay`, `Http01ChallengePort=80`
- `letsencrypt-ip`이면 `CertificateAuthority=letsencrypt-external-dependency`이며 `LetsEncryptTermsAccepted=True`
...
`letsencrypt-ip`을 승인해 실제 생성할 때는 같은 계획 옵션을 `-Execute` 명령에도 그대로 넣습니다. 옵션이 달라지면 승인 계획의 SHA-256이 바뀌므로 도구가 중단합니다.
...
첫 계획과 확인 문구 사이에 Git commit, 계정, 가격, 인증서 모드 또는 기존 자원 상태가 바뀌면 승인 SHA-256이 달라져 생성 전에 중단합니다. 기본값은 서울 `ap-northeast-2a`, 자동 snapshot과 경보 활성화, 브라우저 SSH만 허용, `self-signed` 인증서입니다. 설치 과정은 Docker, 고정 이미지, QFieldCloud 전용 DB 구조, 좌표 변환 격자(PROJ-data), 선택한 인증서와 로컬 관리자를 준비한 뒤 작은 시험 프로젝트로 QGIS 3 worker(백그라운드 작업)를 자동 검증합니다. `letsencrypt-ip`에서는 최초 공인 인증서 발급·검증 또는 갱신 timer 활성화가 실패하면 자체서명 인증서로 성공 처리하지 않습니다.
...
`create-stack` 요청은 CloudFormation **termination protection(삭제 방지)**을 생성 시작부터 켜고, stack policy(스택 업데이트 안전 정책)로 모든 업데이트를 거부합니다. 성공과 실패 스택 모두 보호될 수 있습니다. 모든 완료 조건이 통과하면 배포 도구는 스택 상태와 최초 제공 인증서의 SHA-256 fingerprint(인증서 지문)를 확인합니다. 출력의 `BootstrapStatus`가 `verified-by-wait-condition`, `CertificateSha256`이 64자리, `CertificateMode`가 승인값, `TerminationProtection`이 `enabled`인지 확인합니다. `letsencrypt-ip`의 인증서는 이후 자동 갱신 때 지문이 바뀌므로 이 최초 지문을 영구 pin(고정 신뢰값)으로 사용하지 않으며, 현재 공인 CA 체인과 IP SAN은 상태 도구가 매번 다시 검증합니다.

### 실패하면 자동 삭제하지 않음

이 배포는 실패 원인을 조사할 수 있도록 CloudFormation 자동 롤백을 끕니다. 따라서 설치, `letsencrypt-ip` 최초 발급, worker, 최초 백업 또는 격리 복원시험이 실패해 `CREATE_FAILED`가 되면 인스턴스, 고정 IP 또는 snapshot이 남아 **즉시 계속 과금**될 수 있습니다. 삭제 방지도 생성 시작부터 켜져 있으므로 터미널을 닫거나 일반 삭제를 한 번 누르는 것만으로 없어지지 않습니다. 배포 스크립트도 실패한 자원을 자동 삭제하지 않습니다. 성공 뒤 정기 갱신만 실패한 경우에는 검증을 통과한 이전 인증서를 그대로 두고 root 전용 실패 표식을 만들며, 자체서명 인증서로 바꿔 성공처럼 보이게 하지 않습니다.
...
아무 옵션도 추가하지 않으면 기존 검증 경로를 유지합니다. 주소는 고정 IPv4를 넣은 `https://<고정IPv4>.sslip.io/`입니다. `sslip.io`는 AWS나 QFieldCloud가 운영하지 않는 제3자 wildcard DNS 서비스입니다.

설치기는 이 호스트 이름으로 365일짜리 **자체서명 TLS 인증서**를 만듭니다. 통신은 암호화되지만 공인 인증기관이 서버 신원을 보증하지 않으므로 브라우저와 QField는 경고합니다. 상태 도구는 지문, 유효기간과 호스트 이름을 확인하며 만료되면 실패합니다. 자동 갱신은 없으므로 만료 전에 데이터를 외부로 보존하고 새로 검토한 스택으로 교체하거나 파일럿을 삭제해야 합니다. 2026-08-23 실제 AWS 빈 서버 검증은 이 모드에서 수행했습니다.

### 선택 기능: `letsencrypt-ip`

`-CertificateMode letsencrypt-ip -AcceptLetsEncryptTerms`를 함께 지정하면 주소는 제3자 DNS 이름이 아닌 `https://<고정IPv4>/`가 됩니다. 설치기는 digest로 고정한 Certbot `5.7.0`을 사용해 Let’s Encrypt production ACME endpoint에 `shortlived` profile과 HTTP-01 challenge를 요청합니다. IP 주소 인증서는 [Let’s Encrypt 공식 안내](https://letsencrypt.org/2026/01/15/6day-and-ip-general-availability.html)에 따라 **160시간** 동안 유효하고, HTTP-01 검증에는 이미 공개된 TCP 80을 사용합니다. DNS-01이나 새 AWS 자원은 만들지 않습니다.

Nginx를 먼저 시작해 HTTP-01 파일을 제공하기 위해 설치 중에 임시 자체서명 IP 인증서가 잠시 만들어질 수 있습니다. 이는 challenge를 위한 bootstrap 재료일 뿐 최종 인증서나 성공 fallback이 아닙니다. 검증된 Let’s Encrypt 인증서가 `current`로 전환되고 실제 HTTPS 확인까지 끝나기 전에는 CloudFormation 성공을 보고하지 않습니다.

서버의 systemd timer는 00:00·06:00·12:00·18:00 UTC를 기준으로 **6시간마다**, 서버 쏠림을 줄이기 위한 최대 **45분 무작위 지연** 뒤 갱신을 확인합니다. 재부팅으로 실행 시각을 놓치면 `Persistent=true` 설정이 다음 부팅 뒤 다시 실행합니다. 후보 인증서는 다음 조건을 모두 통과해야 합니다.

- 운영체제 신뢰 저장소에서 공인 CA chain(인증 경로)이 유효함
- SAN(인증서 대상)에 현재 고정 IPv4가 들어 있음
- 인증서와 개인키가 서로 일치함
- 만료까지 **48시간보다 많이** 남음
- Nginx 설정 검사와 실제 HTTPS 응답이 성공함

검증된 인증서와 개인키는 새 release 폴더에 root 전용 `0600`으로 복사합니다. 그 뒤 `certs/current` symbolic link(현재 인증서를 가리키는 링크)만 원자적으로 바꾸고 Nginx를 reload합니다. 새 인증서 제공이나 HTTPS 확인이 실패하면 링크를 이전 release로 되돌리고 실패로 기록합니다. 최초 발급이 실패하면 CloudFormation 완료 신호를 보내지 않으며, 정기 갱신이 실패하면 아직 유효한 이전 인증서를 유지합니다. **두 경우 모두 자체서명 인증서로 바꿔 성공 처리하지 않습니다.**

ACME 계정·발급 이력·작업 로그는 `/opt/qfieldcloud/state/certbot*` 아래 root 전용 상태입니다. 개인키와 ACME 계정 상태는 일반 애플리케이션 백업에 넣지 않습니다. 서버 전체 snapshot에는 포함될 수 있으므로 snapshot 접근 권한도 Secret과 같은 수준으로 제한해야 합니다. 별도 서버로 복구할 때는 데이터를 복원한 뒤 그 서버의 현재 IP로 새 인증서를 발급하는 절차가 필요합니다.

이 모드는 추가 AWS 자원이나 추가 AWS 직접 비용을 만들지 않지만 Let’s Encrypt CA, 공개 인터넷, TCP 80 도달 가능성과 발급 rate limit에 의존합니다. CA 장애나 제한 때문에 발급·갱신이 실패할 수 있으며 반복 수동 실행은 제한을 악화시킬 수 있습니다.

> [!WARNING]
> `letsencrypt-ip` 코드는 정적 계약 검사를 통과하도록 구현했지만 **현재 AWS 스택에는 적용하지 않았고 실제 최초 발급·자동 갱신 종단 간 시험도 아직 완료하지 않았습니다.** 검증 전에는 일반 사용자나 실제 업무 데이터를 위한 운영 TLS로 간주하지 마세요.
...
4. 서버 터미널에서 다음 명령을 실행합니다. 개인키는 출력하지 않습니다.

   ```bash
   sudo openssl x509 \
     -in /opt/qfieldcloud/state/certs/current/fullchain.pem \
     -noout -subject -issuer -dates -ext subjectAltName -fingerprint -sha256
   ```

5. `self-signed`이면 자신의 브라우저 인증서 세부정보에서 SHA-256 fingerprint와 `IP.sslip.io` 호스트 이름을 비교하고, 정확히 같을 때만 경고를 일시적으로 허용합니다.
6. `letsencrypt-ip`이면 브라우저가 공인 CA chain을 정상 신뢰하고 SAN의 IP 주소가 접속한 고정 IPv4와 같은지 확인합니다. 브라우저 경고가 있거나 다른 IP가 보이면 로그인하지 말고 중단합니다.
...
로컬 상태 도구는 `self-signed`를 무조건 허용하지 않고 스택의 인증된 완료 신호에 담긴 SHA-256 fingerprint와 서버 인증서가 정확히 같을 때만 연결합니다. `letsencrypt-ip`에서는 갱신마다 지문이 바뀌므로 fingerprint를 영구 고정하지 않고 운영체제의 공인 CA 신뢰, 현재 유효기간과 IP SAN 일치를 검사합니다. 어느 모드든 검증이 다르면 중단하세요. 민감하거나 실제 업무용 데이터를 이 공개 파일럿에 넣지 마세요.

## 6. 상태와 관리자 계정 확인

### 로컬 PC에서 생성 완료 조건과 상태 주소 확인

다음 명령은 스택이 `CREATE_COMPLETE`인지 확인하고 선택한 인증서 모드를 스택 출력과 대조합니다. `self-signed`에서는 설치 완료 신호의 SHA-256 fingerprint와 실제 서버 인증서를 비교하고, `letsencrypt-ip`에서는 공인 CA chain·유효기간·IP SAN을 검증합니다. 이어서 Lightsail 실행 상태와 외부 HTTPS 상태 주소의 DB·객체 저장소 상태를 확인합니다.
...
`BootstrapSource=expected-revision-and-sha256-matched`도 확인합니다. `self-signed`이면 `CertificateTrust=self-signed-fingerprint-matched`, `letsencrypt-ip`이면 `CertificateTrust=public-ca-chain-valid`와 `CertificateIdentity=public-ip-san-matched`여야 합니다. 공인 모드의 `CertificateRenewal=scheduled-by-verified-bootstrap`은 최초 설치 때 timer 활성화를 검증했다는 뜻이지 방금 갱신을 실행했다는 뜻은 아닙니다.

스택 모드에서 성공하면 검토한 bootstrap commit·파일, 새 스택 생성 때 자동 worker 시험, 최초 로컬 백업, 최신 백업의 격리 schema·storage 복원시험과 전체 health가 완료 조건을 통과했고, 현재 인증서·HTTPS·DB·storage가 응답함을 뜻합니다. 다만 worker, 복원과 인증서 갱신을 그 순간 다시 실행한 결과는 아니므로 현재 전체 상태는 서버 안의 다음 명령으로 확인합니다.
...
JSON 결과의 `overall`이 `ok`, `runtime_provenance`가 `verified-pinned-installer-files`, `runtime_images`가 `verified-pinned-image-objects`, `protected_state_permissions`가 `root-only`인지 먼저 확인합니다. `transformation_grids`는 `installed-and-verified`, `restore_test_orphans`와 `maintenance_failures`는 `clear`여야 합니다. 이는 설치 commit의 원본 파일과 현재 실행 파일이 같고, 각 컨테이너가 manifest에 고정한 이미지 객체를 실제로 실행하며, 좌표 변환 격자·복원시험 임시 자원·미해결 유지보수 실패 표식까지 다시 확인하고, Secret·개인키 파일이 서버 root만 읽도록 유지된다는 뜻입니다. DB·객체 저장소·SMTP·cache·app·Nginx·worker·작업 스케줄러도 모두 `running`이어야 합니다.

인증서는 `certificate_mode`에 따라 다음 값을 확인합니다.

| 모드 | `tls_certificate` | `certificate_renewal` | 추가 확인 |
|---|---|---|---|
| `self-signed` | `current-hostname-and-fingerprint-matched` | `not-applicable-self-signed` | `IP.sslip.io` 호스트 이름·현재 지문·유효기간 일치 |
| `letsencrypt-ip` | `public-ca-ip-san-current` | `scheduled-and-healthy` | 공인 CA chain, 현재 고정 IPv4 SAN, 인증서·개인키·Nginx live 인증서 일치, 48시간 초과 여유, timer 활성화 |

공인 모드에서는 `certificate_not_after`, `certificate_last_check_at`, `certificate_last_renewal_at`도 함께 기록됩니다. 마지막 확인이 오래됐거나 root 전용 `last-certificate-renewal-failure`가 있으면 전체 상태는 성공이 아닙니다. 이 필드는 Secret이나 개인키를 출력하지 않습니다.
...
기존 식물이력관리 DB는 포함하지 않습니다. `letsencrypt-ip`의 ACME 계정·인증서 개인키·Certbot 작업·로그 상태도 일반 애플리케이션 백업에 포함하지 않습니다. 별도 서버 복구 시 그 서버의 현재 고정 IP로 새 인증서를 발급해야 합니다. 완료 폴더를 덮어쓰거나 자동 삭제하지 않으므로 백업이 쌓이면 80GB 디스크가 가득 찰 수 있습니다. 백업에는 QFieldCloud Secret 사본이 있고 파일 자체를 별도로 암호화하지 않으므로 접근 권한을 엄격히 제한해야 합니다.
...
- **기본 자체서명 TLS와 제3자 DNS:** `self-signed`에는 공인 서버 신원 보장이 없고 `sslip.io`의 가용성과 DNS 처리에 의존합니다. 자동 갱신도 없으므로 365일 만료 전에 검토된 새 스택으로 교체하거나 파일럿을 삭제해야 합니다.
- **선택 공인 인증서의 외부 의존:** `letsencrypt-ip`은 Let’s Encrypt CA, 공개 인터넷, TCP 80 도달 가능성과 rate limit에 의존합니다. 160시간 인증서는 갱신 자동화가 멈추면 빠르게 만료됩니다.
- **공인 모드 실제 검증 전:** 코드와 정적 계약 검사는 실제 발급, 시간 경과 갱신, CA 장애와 재부팅 복구를 증명하지 않습니다. 현재 AWS 스택은 이 모드가 아니며 실제 발급·갱신 종단 간 검증 전입니다.
...
- [Let’s Encrypt 160시간·IP 주소 인증서](https://letsencrypt.org/2026/01/15/6day-and-ip-general-availability.html)
- [Certbot의 IP 주소 인증서 사용법](https://letsencrypt.org/2026/03/11/shorter-certs-certbot/)
- [Certbot `5.7.0` 공식 릴리스](https://github.com/certbot/certbot/releases/tag/v5.7.0)
- [Let’s Encrypt 약관·정책 저장소](https://letsencrypt.org/repository/)
