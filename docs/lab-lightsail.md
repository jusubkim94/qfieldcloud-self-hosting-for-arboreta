...
- **완료:** 인스턴스 실행, 삭제 방지 활성화, 자체서명 인증서의 호스트 이름·SHA-256 지문·유효기간 일치, 외부 HTTPS의 DB·객체 저장소 `ok`, 서버 내부 전체 상태 `overall=ok`, QGIS `3.44.13-Solothurn` worker의 프로젝트 처리·패키징·임시 컨테이너 정리 `passed`를 확인함
...
이전 파일럿에서는 내부 설치와 모든 검증이 끝난 뒤 하위 설치 명령이 표준입력에 남은 CloudFormation 완료 구문을 소비해 성공 신호를 수동 보완해야 했습니다. `</dev/null`로 하위 명령의 입력을 분리한 수정본은 2026-08-23 실제 빈 서버 재배포에서 같은 WaitCondition(완료 대기 조건)에 자동 성공 신호를 보내고 `CREATE_COMPLETE`가 되는 것까지 확인했습니다. 위 로컬 백업과 격리 복원시험은 현재 서버 안에서 수행한 검사이므로 서버 자체를 잃는 재해에 대비한 별도 서버 복구 검증은 아닙니다. 실제 AWS에서 실행하지 않은 나머지 범위는 성공했다고 보지 않습니다.
