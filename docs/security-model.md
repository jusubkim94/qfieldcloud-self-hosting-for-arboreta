...
이 문서는 브라우저 원클릭 `lab-lightsail` 파일럿이 무엇을 보호하고 어떤 위험을 받아들이는지 설명합니다. 이 구성은 중요 운영용 보안 기준을 충족한다고 보장하지 않습니다.

## 현재 검증 상태

- 릴리스별 CloudFormation artifact는 아직 S3에 게시되지 않았습니다.
- 새 Quick Create 경로는 실제 AWS에서 종단 간 검증하지 않았습니다.
...
5. 릴리스별 S3 경로와 checksum이 검증된 템플릿만 실행합니다.

CloudFormation 또는 Lightsail 권한이 부족하면 AWS의 권한 오류로 설치가 중단됩니다. 이 프로젝트는 부족한 권한을 우회하거나 현재 사용자를 자동 승격하지 않습니다.

## 공급망과 배포 artifact

Quick Create가 사용할 템플릿은 다음 조건을 만족해야 합니다.

- `main`, `master`, `latest`가 아닌 릴리스별 S3 객체
- 게시 전에 생성한 SHA-256과 게시된 객체의 SHA-256이 일치
- QFieldCloud 릴리스, Git commit과 `linux/amd64` 이미지 digest가 고정
- bootstrap 파일을 다운로드한 뒤 SHA-256을 확인하고 일치할 때만 실행
- 만료되는 presigned URL이나 인증정보가 포함된 URL을 사용하지 않음

현재 S3 객체와 Quick Create URL은 아직 게시되지 않았습니다. 사용 가능한 버튼처럼 보이는 placeholder를 실행해서는 안 됩니다.

## Secret 처리

- 템플릿 입력값과 Quick Create URL에 비밀번호, 토큰, Access Key, 개인키 또는 DB 접속 문자열을 넣지 않습니다.
...
공인 인증서 모드는 고정된 Certbot 이미지, 현재 static IPv4 SAN, CA chain, 인증서·개인키 일치와 만료 여유를 검사한 뒤에만 적용합니다. 실제 원클릭 경로의 최초 발급과 시간 경과 갱신은 아직 검증하지 않았습니다.
