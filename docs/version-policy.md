...
> 현재 `v0.1.2` 완성 artifact가 저장소에 포함되어 있습니다. 원본 템플릿에는 릴리스 생성 과정에서 바꿀 표식이 있으므로 직접 배포할 수 없습니다. `v0.1.2` 실제 시험은 공인 IP 인증서 적용까지 성공한 뒤 worker 내부 Nginx Host 검사에서 실패했습니다. 다음 릴리스의 Docker 전용 worker API 경로는 아직 실제 AWS 재시험 전입니다.
