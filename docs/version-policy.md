...
> 현재 `v0.1.4` 완성 artifact가 저장소에 포함되어 있습니다. 원본 템플릿에는 릴리스 생성 과정에서 바꿀 표식이 있으므로 직접 배포할 수 없습니다. `v0.1.3` 자체서명 경로와 Let’s Encrypt 공인 IP 인증서가 기본인 `v0.1.4` 경로는 각각 실제 AWS `CREATE_COMPLETE`까지 통과했습니다. `v0.1.4` 고정값과 검증 범위는 [실제 AWS 검증 기록](verification/lab-lightsail-v0.1.4-2026-08-25.md)에 있습니다.
...
이 파일럿에는 자동 스냅샷, 애플리케이션 백업 또는 데이터 보존 업데이트가 없습니다. `v0.1.4` 새 스택 생성은 실제 AWS에서 검증했지만 데이터 이전이 포함된 업데이트나 제자리 교체는 지원하거나 검증하지 않았습니다. 자세한 사용자 절차는 [업데이트 실행서](runbooks/update.md)와 [롤백 실행서](runbooks/rollback.md)를 따릅니다.
