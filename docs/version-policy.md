...
> 현재 `v0.1.1` 완성 artifact가 저장소에 포함되어 있습니다. 원본 템플릿에는 릴리스 생성 과정에서 바꿀 표식이 있으므로 직접 배포할 수 없습니다. `v0.1.0`의 2026-08-24 수동 업로드 생성은 `create_project` worker 검증에서 실패했으며, 진단 보존을 추가한 `v0.1.1`은 아직 실제 AWS 재시험 전입니다.
