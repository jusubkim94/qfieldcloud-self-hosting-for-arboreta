...
> 현재 릴리스별 S3 artifact와 Quick Create URL은 게시되지 않았습니다. 저장소의 원본 템플릿에는 릴리스 생성 과정에서 바꿀 표식이 있으므로 직접 배포할 수 없습니다. 단순화한 배포 artifact 흐름도 실제 AWS에서 아직 검증하지 않았습니다.
...
각 묶음은 `releases/<고정 버전>/...` 형태의 새 S3 키에 올리고 S3 Versioning의 `VersionId`를 URL에 포함해야 합니다. 이미 게시한 같은 릴리스 객체를 다른 내용으로 덮어쓰지 않습니다.

Quick Create 링크는 다음 값을 고정해야 합니다.
...
6. 사용자가 승인한 뒤에만 새 S3 객체 버전과 Quick Create 링크를 게시합니다.
