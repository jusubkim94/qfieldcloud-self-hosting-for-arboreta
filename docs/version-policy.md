...
> 현재 `v0.1.4` 완성 artifact가 저장소에 포함되어 있습니다. 원본 템플릿에는 릴리스 생성 과정에서 바꿀 표식이 있으므로 직접 배포할 수 없습니다. `v0.1.3` 자체서명 경로는 실제 AWS `CREATE_COMPLETE`까지 통과했고, Let’s Encrypt 공인 IP 인증서가 기본인 `v0.1.4`는 실제 AWS 재시험 전입니다.
