...
- **실제 AWS 생성 실패를 재현했고, 가장 최근 원인은 고정 QGIS 이미지에 없는 `python` 명령으로 버전을 검증해 종료코드 127이 발생한 것으로 확인·수정했지만 최신 수정안 재배포는 아직 검증하지 않았습니다.** 같은 이미지의 `/usr/bin/python3`으로 고정 QGIS 버전은 직접 확인했습니다. 문서의 명령이 실제 AWS 설치 성공을 보장한다는 뜻은 아닙니다.
